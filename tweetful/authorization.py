...
def get_user_authorization(request_token):
	#redirect the user to authorize the client and get them to give a verification code
...
def get_access_token(request_token, request_secret, verifier):
	#get token to allow us to make requests to the API
	oauth = OAuth1(CLIENT_KEY,
		client_secret = CLIENT_SECRET,
...
def get_stored_credentials():
	#retrieve stored credentials from a json file
